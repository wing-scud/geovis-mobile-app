...
GeoVisFirstPersonCameraController.prototype._onMouseLButtonClicked = function(
  ...
  if (this._direction === TARGET_ROTATE) {
    console.log("222222");
    if (!this._enabled) return;
    console.log("!!!!!!!!!");
...
// 鼠标左键+shift按下开始围绕目标点旋转

// 鼠标左键+shift释放停止旋转，并同步视高
...
GeoVisFirstPersonCameraController.prototype._onKeyUp = function(event) {
  this._direction = DIRECTION_NONE;
};
...
GeoVisFirstPersonCameraController.prototype._onClockTick = function(clock) {
  ...
  if (this._rotating) {
    ...
    if (this._direction === DIRECTION_NONE || this._direction === TARGET_ROTATE)
      return;
