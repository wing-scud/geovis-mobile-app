<template>
  <div>
    <div class="searchContainer black-box">
      ...
      <div class="navBtn">
        <img title="点击检索" src="/images/images/gf/daohang.png" alt="" @click="searchLujing(true)" />
    ...
    <div v-show="searchBox">
      <div class="results" v-if="plans.length">
        <div class="contents">
          <div class="list">
            <button :class="{ item: true, checked: plan.name == item.name }" v-for="(item, index) in plans" :key="index" @click="choosePlan(index)">
              {{ luxian(index) }}
            </button>
          </div>
        </div>
...
export default {
  ...
  methods: {
	...
	/* 根据（type==1？起点：终点）地名搜索得到结果数组dmResults */
    getDM(type) {
    ...
    async searchLujing(flag) {
      ...
      if (this.startPoint.length && this.endPoint.length) {
        ...
        const url = "http://router.project-osrm.org/" + "/route/v1/driving/" + start + ";" + end + "?steps=true"; //Config.LJIP
        this.plans = await fetch(url)
          .then((res) => res.json())
          .then((data) => this.analysisJson(data));
        ...
        for (let i = 0; i < paths.length; i++) {
          for (let j = 0; j < paths[i].coordinates.length - 1; j++) {
            ...
            end = [paths[i].coordinates[j + 1][1], paths[i].coordinates[j + 1][0], 0];
    ...
    addXuxian(linePositions, index, point) {
      ...
      var position = [new GeoVis.Cartesian3.fromDegrees(Number(point[0]), Number(point[1]), 0), new GeoVis.Cartesian3.fromDegrees(route[0], route[1], 0)];
    ...
    choosePlan(index) {
      ...
      for (var j = 0; j < this.plan.instructions.length; j++) this.texts[j] = this.formatText(this.plan.instructions[j]);
