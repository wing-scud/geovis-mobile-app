function getMapVRender() {
  var { GeoVis, mapv: mapV } = window;
  ...
  var MapVRenderer = (function(_baseLayer) {
    function layer(viewer, dataSet, options, mapVLayer) {
      ...
      return baseLayer
        ? ((_this.map = viewer),
          ...
          _this.bindEvent(),
          _this)
        : possibleConstructorReturn(_this);
    }
    ...
    createClass(layer, [
      ...
      },
      {
        ...
        value: function() {
          var map = this.map;
          this.options.methods && (this.options.methods.click && map.off("click", this.clickEvent), this.options.methods.mousemove && map.off("mousemove", this.mousemoveEvent));
        }
      },
      {
        key: "getContext",
        ...
        value: function(e) {
          ...
          if (this.canvasLayer && !this.stopAniamation) {
            ...
            if (this.isEnabledTime()) {
              if (void 0 === e || this.morphing) {
                return; //this.clear(context); //Math.random() > 0.5 ?  : null; //
              } 
            ...
            var _tCoord = {
              transferCoordinate: function(e) {
                var i = [99999, 99999],
                  a = GeoVis.Cartesian3.fromDegrees(e[0], e[1]);
                if (!a) return i;
                var n = scene.cartesianToCanvasCoordinates(a);
                if (!n) return i;
                if (scene.mode === GeoVis.SceneMode.SCENE3D) {
                  if (GeoVis.Cartesian3.angleBetween(scene.camera.position, a) > GeoVis.Math.toRadians(80)) return !1;
                }
                return [n.x, n.y];
              }
            };
            ...
            // WGS84坐标中的位置转换为窗口坐标
            var coord = GeoVis.SceneTransforms.wgs84ToWindowCoordinates(scene, GeoVis.Cartesian3.fromDegrees(0, 0));
