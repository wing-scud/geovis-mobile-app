...
CompositeCoordinateSystem.prototype.dataToPoint = function(data) {
  ...
  var position = GeoVis.Cartesian3.fromDegrees(data[0], data[1]);
  if (!position) {
    return [0, 0];
  }
  // var px = this._GLMap.cartesianToCanvasCoordinates(position);
  var px = GeoVis.SceneTransforms.wgs84ToDrawingBufferCoordinates(this._GLMap,position)
  if (!px) {
    return [0, 0];
  }
...
export default CompositeCoordinateSystem;
