...
export default class Orbit {
    constructor(name, tle) {
        this.name = name;
        this.tle = tle.split("\n");
        this.satrec = satellitejs.twoline2satrec(this.tle[1], this.tle[2]); //返回从两行TLE数据导入的卫星
    }
    ...
    positionECI(time) {
        return satellitejs.propagate(this.satrec, time).position; //返回给定日期和时间的位置和速度向量
    }
    ...
    positionGeodetic(time) {
        const positionEci = this.positionECI(time);
        const gmst = satellitejs.gstime(time); //从julian日期算出greenwich sidereal时间
